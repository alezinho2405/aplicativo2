...
const AreaExterna = ({ navigation }) => {
  const [areaExternaValue, setAreaExternaValue] = useState(0);
  const [usoValue, setUsoValue] = useState(0);

  const consumoMensal = (areaExternaValue * 7 * 30).toFixed(2); // Cálculo de consumo mensal
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity 
        ...
        onPress={() => navigation.navigate('FinalAreaExterna')} // Navegando para a tela FinalAreaExterna
      >
